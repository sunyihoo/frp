...
func (c *Controller) HandleVisitor(m *msg.NatHoleVisitor, transporter transport.MessageTransporter, visitorUser string) {
	if m.PreCheck {
		...
		if !slices.Contains(cfg.allowUsers, visitorUser) && !slices.Contains(cfg.allowUsers, "*") {
			_ = transporter.Send(c.GenNatHoleResponse(m.TransactionID, nil, fmt.Sprintf("xtcp visitor use [%s] not allowed for [%s]", visitorUser, m.ProxyName)))
			return
		}
	...
	case <-time.After(time.Duration(NatHoleTimeout) * time.Second):
		log.Debugf("wait for NatHoleClient timeout, sid [%s]", sid)
		return
	}
	...
	g.Go(func() error {
		// if it's sender, wait for a while to make sure the client has sent the detect messages
...
func (c *Controller) analysis(session *Session) (*msg.NatHoleResp, *msg.NatHoleResp, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("classify clent nat feature error: %v", err)
	}
	...
	mode, index, cBehavior, vBehavior := c.analyzer.GetRecommandBehavior(session.analysisKey, cNatFeature, vNatFeature)
	...
	log.Debugf("sid [%s] client detect dehavior: %+v", session.sid, cResp.DetectBehavior)
