...
var (
	...
	mode4Behavior = []lo.Tuple2[RecommandBehavior, RecommandBehavior]{
		lo.T2(
			RecommandBehavior{Role: DetectRoleSender, PortsRandomNumber: 1000, SendDelayMs: 2000},
			RecommandBehavior{Role: DetectRoleReceiver, TTL: 7, ListenRandomPorts: 256, PortsRangeNumber: 2},
		),
		lo.T2(
			RecommandBehavior{Role: DetectRoleSender, PortsRandomNumber: 1000, SendDelayMs: 2000},
			RecommandBehavior{Role: DetectRoleReceiver, TTL: 4, ListenRandomPorts: 256, PortsRangeNumber: 2},
		),
		lo.T2(
			RecommandBehavior{Role: DetectRoleSender, PortsRandomNumber: 1000, SendDelayMs: 2000},
...
func getBehaviorScoreByMode(mode int, defaultScore int) []*BehaviorScore {
	return getBehaviorScoreByMode2(mode, defaultScore, defaultScore)
}

func getBehaviorScoreByMode2(mode int, senderScore, receiveScore int) []*BehaviorScore {
	behaviors := getBehaviorByMode(mode)
...
func NewMakeHoleRecords(c, v *NatFeature) *MakeHoleRecords {
	scores := make([]*BehaviorScore, 0)
	easyCount, hardCount, portsChangedRegularCount := ClassifyFeatureCount([]*NatFeature{c, v})
	appendMode0 := func() {
		switch {
		case c.PublicNetWork:
			scores = append(scores, getBehaviorScoreByMode2(DetectMode0, 0, 1)...)
		case v.PublicNetWork:
			scores = append(scores, getBehaviorScoreByMode2(DetectMode0, 1, 0)...)
		default:
			scores = append(scores, getBehaviorScoreByMode(DetectMode0, 0)...)
	...
	case hardCount == 1 && portsChangedRegularCount == 1:
		scores = append(scores, getBehaviorScoreByMode(DetectMode1, 0)...)
		scores = append(scores, getBehaviorScoreByMode(DetectMode2, 0)...)
		appendMode0()
	...
	case hardCount == 2 && portsChangedRegularCount == 2:
		scores = append(scores, getBehaviorScoreByMode(DetectMode3, 0)...)
		scores = append(scores, getBehaviorScoreByMode(DetectMode4, 0)...)
	case hardCount == 2 && portsChangedRegularCount == 1:
		scores = append(scores, getBehaviorScoreByMode(DetectMode4, 0)...)
	default:
		// hard to make hole, just trying it out
		scores = append(scores, getBehaviorScoreByMode(DetectMode0, 1)...)
		scores = append(scores, getBehaviorScoreByMode(DetectMode1, 1)...)
		scores = append(scores, getBehaviorScoreByMode(DetectMode3, 1)...)
	}
...
func (a *Analyzer) GetRecommandBehavior(key string, c, v *NatFeature) (mode, index int, _ RecommandBehavior, _ RecommandBehavior) {
	a.mu.Lock()
	...
	case DetectMode4:
		// Regular ports change is always the sender
